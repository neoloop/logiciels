...
///   - "Service émetteur"    → service that issued the order (optional)
///   - "Service de facturation" → service that gets invoiced (optional)
...
enum SeditImportService {
    ...
    private static let emetteurHeaders = ["service emetteur"]
    private static let facturationHeaders = ["service de facturation"]
    ...
    static func importWorkbook(at url: URL) throws -> [SeditCommandeLine] {
        ...
        let emetteurColumn = ExcelSheetParsing.firstMatch(emetteurHeaders, in: columns)
        let facturationColumn = ExcelSheetParsing.firstMatch(facturationHeaders, in: columns)
        ...
        for row in rows.dropFirst() {
            ...
            let serviceEmetteur = emetteurColumn.flatMap { cells[$0] }.flatMap { Int($0.trimmingCharacters(in: .whitespacesAndNewlines)) }
            let serviceFacturation = facturationColumn.flatMap { cells[$0] }.flatMap { Int($0.trimmingCharacters(in: .whitespacesAndNewlines)) }
            ...
            results.append(
                SeditCommandeLine(
                    ...
                    serviceFacturation: serviceFacturation,
